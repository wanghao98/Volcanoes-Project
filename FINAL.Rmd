...
For Xgboost, we changed the tuning parameters, eta and subsample to avoid overfitting; min-child-weight to make the algorithm more conservative. 


### Neural network:

We implemented similar algorithm for Nerual network as in the classification section.

**Figure 12** shows the result of the second neural network model which is designed to classify whether an image contains at least one volcano. The model, similar to the first one, used 20% of the training data as validation data. 
...
Our model prediction results for Poisson count prediction are summarized in table below. 
...
```{r echo= TRUE, out.width="250px", fig.cap="The Marginal Screening index for classification", fig.pos='H'}
...
Compared to structured data, the images used in this project are unstructured as volcanoes can occur in any pixels, this makes featuring difficult. Models such as lasso, logistic, random forest, require more of predefined features. In this project, convolution neural network works best as it includes different hidden layers and pools layers.  
...
In addition, dimension reduction methods such as single value decomposition was not performed. As image data may include several noises, performing dimension reduction may have improvment in the prediction. As the dataset in this project was quite large, simply using `svd` function to perform dimension reduction may take a lot of time. For future improvement of this project, we could try faster packages to perform partial singular value decompositions and perform predicton. 
