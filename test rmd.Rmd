...
### Logistic Regression
Before applying logistic regression, we tried Marginal Screening and lasso to apply variable selection. For Marginal Screenign part, by modeling each pixel in the *train_images* using glm function, we selected 400 pixels based on prediction accuracy. For Lasso regression, we selected pixels by using lambda.min and 470 pixels were selected. Then, we applyed logistic regression on those 400 and 470 pixels repectively. We then recorded their accuracies and AUC. 
### Neural Network  
