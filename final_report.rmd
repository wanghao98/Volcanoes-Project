...
rfmodel = randomForest(train.x, y=train.y, nodesize = 70, mtry = 80,ntree = 200,  importance=TRUE)
```

```{r}
rfpred_res = predict(rfmodel, as.matrix(test_data))
table(rfpred_res, test_y[,1])
rfpred = predict(rfmodel, as.matrix(test_data), type="prob")
rfpred1 = rfpred[,2]

auc(test_y[,1], rfpred1)
F1_Score(test_y[,1], rfpred_res)
...
predict number of colvanoes
```{r}
train.x = as.matrix(train_data)
train.y=as.matrix(train_y[,4])
test.y=as.matrix(test_y[,4])
